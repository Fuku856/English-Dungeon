...
迷宮を探索し、英語の力で道を切り開け！
8bitレトロスタイルの英語学習ダンジョンRPGです。モンスターとのバトルを通じて、単語・文法・リスニング・会話力を鍛えながら、ゴールを目指しましょう!
...
### データの保存と読み込み
ゲームの進行状況は、ブラウザへの保存（Save/Load）と、ファイルへの書き出し（Export/Import）の2通りの方法で管理できます。

*   **ブラウザ保存 (Save/Load)**: 手軽に続きから遊びたい場合
    *   **Save (保存)**: ゲーム中のメニュー（Bボタン）から「SAVE」を選択またはタップすると、ブラウザのローカルストレージに保存されます。
    *   **Load (読み込み)**: タイトル画面の「LOAD GAME」を選択すると、保存した地点から再開できます。
    ※ ブラウザのキャッシュ削除などでデータが消える可能性があります。

*   **ファイル管理 (Export/Import)**: バックアップや別端末への移動
    *   **Export (書き出し)**: ゲーム中のメニュー（Bボタン）から「EXPORT」を選択またはタップすると、セーブデータをJSONファイルとしてダウンロードできます。
    *   **Import (読み込み)**: タイトル画面の「IMPORT」を選択し、ダウンロードしたJSONファイルをアップロードすると、データを復元して再開できます。
